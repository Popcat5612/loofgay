...
-- 시작할 때 clicktp/clickdel 바인드 자동 제거
for i = #binds, 1, -1 do
...
function notify(text,text2,length)
	if getgenv().NOTIFY == false then return end
	task.spawn(function()
...
CMDs[#CMDs + 1] = {NAME = 'teleport/tp/to/goto (player, dummy)', DESC = '플레이어한태 순간이동합니다(이름지정,dummy한태 지정시 더미한태 이동)'}
CMDs[#CMDs + 1] = {NAME = 're', DESC = '리셋하고 그자리에서 탤래포트를합니다'}
CMDs[#CMDs + 1] = {NAME = 'reset', DESC = '강제리셋'}
CMDs[#CMDs + 1] = {NAME = 'fakeclip/banstart alt name kill totalkill', DESC = '원하는사람을 핵쟁이로 누명을 씌울수있는 가짜클립을 실행합니다'}
CMDs[#CMDs + 1] = {NAME = 'antideathcountergui/adcgui', DESC = '언노운 안티 데스카운터 GUI를 엽니다(데스카운터 우회)'}
CMDs[#CMDs + 1] = {NAME = 'crabadd/crabspawn', DESC = '공섭에서 개보스를 로벅스를주고 소환할수있습니다(299롭) 만약 소환후 잡고 다시실행하면 개보스로 변합니다'}
...
CMDs[#CMDs + 1] = {NAME = 'antiragdoll/antirg', DESC = '휘청덤 상태를 방지합니다'}
CMDs[#CMDs + 1] = {NAME = 'unantiragdoll/unantirg', DESC = '휘청덤 방지를 비활성화합니다'}
...
CMDs[#CMDs + 1] = {NAME = 'walkfling/vspoof', DESC = '속도값을 위조하여 상대를 날려버립니다 (WalkFling)'}
CMDs[#CMDs + 1] = {NAME = 'unwalkfling/unvspoof', DESC = 'WalkFling을 비활성화합니다'}
CMDs[#CMDs + 1] = {NAME = 'walkdeathfling/wdf', DESC = 'Touch Fling 기능을 활성화합니다'}
CMDs[#CMDs + 1] = {NAME = 'unwalkdeathfling/unwdf', DESC = 'Touch Fling 기능을 비활성화합니다'}
CMDs[#CMDs + 1] = {NAME = 'deathflinggui', DESC = 'Touch Fling GUI를 토글합니다'}
CMDs[#CMDs + 1] = {NAME = 'emotedash/edash', DESC = '이모트대쉬를 사용할수 있습니다'}
CMDs[#CMDs + 1] = {NAME = 'unemotedash/unedash', DESC = '이모트 대시를 비활성화합니다'}
CMDs[#CMDs + 1] = {NAME = 'walkdeathfling/wdf', DESC = '데스플링을 킵니다'}
CMDs[#CMDs + 1] = {NAME = 'unwalkdeathfling/unwdf', DESC = '데스플링을 끕니다'}
CMDs[#CMDs + 1] = {NAME = 'deathflinggui', DESC = '데스플링 GUI를 킵니다(키보드 H)'}
wait()
...
addcmd('fling', {}, function(args, speaker)
	...
	if target and target.Character and lp.Character then
		...
		connection = game:GetService("RunService").Heartbeat:Connect(function()
			...
			local predictedPos = targetHrp.CFrame + targetVelocity * 0.1
			
			-- 초고속 Velocity + 상대 속도 예측
			hrp.Velocity = Vector3.new(50000, 50000, 50000) + targetVelocity * 2
			hrp.RotVelocity = Vector3.new(50000, 50000, 50000)
			
			-- 예측 위치로 이동
			hrp.CFrame = predictedPos * CFrame.Angles(
		...
		notify('Fling', target.Name .. '에게 Fling!')
...
local function isPlayerInAir(character)
	if not character then return false end
	local humanoid = character:FindFirstChildWhichIsA("Humanoid")
	if not humanoid then return false end
	
	-- Humanoid 상태 체크 (Freefall, Jumping, Flying 등)
	local state = humanoid:GetState()
	if state == Enum.HumanoidStateType.Freefall or state == Enum.HumanoidStateType.Jumping or state == Enum.HumanoidStateType.Flying then
		return true
	end
...
addcmd('loopfling', {}, function(args, speaker)
	...
	for i, t in pairs(targets) do
		if i == 1 then
			...
			notifyMsg = notifyMsg .. ', ' .. t.Name
		end
	end
	notify('LoopFling', notifyMsg .. ' LoopFling 시작!')
	...
	task.spawn(function()
		while getgenv().LoopFlingActive do
			...
			-- 타겟 유효성 확인
			if target and target.Character and target.Parent == Players then
				...
				if myChar then
					...
					if myHrp and targetHrp then
						-- 공중에 있으면 스킵
						if not isPlayerInAir(target.Character) then
							-- fling 실행
							local savedPos = myHrp.CFrame
							
							local connection
							connection = game:GetService("RunService").Heartbeat:Connect(function()
								if not getgenv().LoopFlingActive or not target or not target.Character then
									if connection then
										connection:Disconnect()
									end
									return
								...
								if not currentHrp or not currentTargetHrp then
									if connection then
										connection:Disconnect()
									end
									return
								...
								-- 타겟의 상대 속도 예측
								local targetVelocity = currentTargetHrp.Velocity
								local predictedPos = currentTargetHrp.CFrame + targetVelocity * 0.1
								
								-- 초고속 Velocity + 상대 속도 예측
								currentHrp.Velocity = Vector3.new(50000, 50000, 50000) + targetVelocity * 2
								currentHrp.RotVelocity = Vector3.new(50000, 50000, 50000)
								
								-- 예측 위치로 이동
								currentHrp.CFrame = predictedPos * CFrame.Angles(
									math.rad(math.random(0, 360)),
									math.rad(math.random(0, 360)),
									math.rad(math.random(0, 360))
								)
...
addcmd('unloopfling', {}, function(args, speaker)
	getgenv().LoopFlingActive = false
	getgenv().LoopFlingTargets = {}
	notify('LoopFling', 'LoopFling 중지됨')
end)

-- Phantasm Breaker GUI
addcmd('phantasmbreakergui', {'phbgui'}, function(args, speaker)
	...
	btn.Text = "판타즘세미갓 브래이커"
	btn.TextColor3 = Color3.new(1, 1, 1)
	btn.Font = Enum.Font.GothamBold
	btn.TextSize = 12
	
	-- 순차 재생 핵심 로직
	local function runSequential()
		while isSpamming do
			for _, id in ipairs(animIds) do
				...
				currentTrack = Humanoid:LoadAnimation(a)
				...
				-- 애니메이션이 끝날 때까지 대기
				task.wait(0.1)
				while currentTrack.IsPlaying and isSpamming do
					task.wait(0.1)
				end
	...
	btn.MouseButton1Click:Connect(function()
		...
		if isSpamming then
			...
			task.spawn(runSequential)
		else
			btn.Text = "판타즘세미갓 브래이커(off)"
			btn.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
			if currentTrack then 
				pcall(function() currentTrack:Stop() end)
			end
...
--커맨드추가
addcmd('test', {}, function(args, speaker)
    print("aaa")
    wait(1)
    notify('완료', 'aaa 출력이 완료되었습니다', 3)
end)

addcmd('rj', {'rejoin'}, function(args, speaker)
    notify('Rejoin', '서버로 다시 접속 중입니다...', 3)
    wait(0.5)
    game:GetService("TeleportService"):Teleport(game.PlaceId)
end)

addcmd('teleport', {'tp', 'to', 'goto'}, function(args, speaker)
    if not args[1] then
    ...
    local actualTargetName = nil
    
    -- "dummy"인 경우 NPC 찾기
    if string.lower(target) == 'dummy' then
        for _, v in pairs(workspace:GetDescendants()) do
            if v:IsA("Model") and getRoot(v) and v:FindFirstChildWhichIsA("Humanoid") and Players:GetPlayerFromCharacter(v) == nil then
                targetRoot = getRoot(v)
                actualTargetName = v.Name
                break
            end
        end
        ...
        for _, v in pairs(Players:GetPlayers()) do
            if string.lower(v.Name):find(string.lower(target), 1, true) or string.lower(v.DisplayName):find(string.lower(target), 1, true) then
                targetPlayer = v
                break
            end
        end
        ...
        if not targetPlayer then
            notify('Error', '플레이어를 찾을 수 없습니다: ' .. target)
            return
        end
        ...
        actualTargetName = targetPlayer.DisplayName ~= "" and targetPlayer.DisplayName or targetPlayer.Name
    end
    ...
    if not charRoot or not targetRoot then
        notify('Error', 'Character를 찾을 수 없습니다')
        return
    end
    
    charRoot.CFrame = targetRoot.CFrame + Vector3.new(3, 0, 0)
    notify('Teleport', (actualTargetName or target) .. '에게 이동했습니다', 2)
end)
...
addcmd('re', {'respawn', 'refresh'}, function(args, speaker)
    local char = speaker.Character
    ...
    task.spawn(function()
        -- 캐릭터가 바뀌거나 새로 생성될 때까지 대기
        local newChar = speaker.CharacterAdded:Wait()
        local newRoot = newChar:WaitForChild("HumanoidRootPart", 10)
        
        if newRoot then
            -- 가강전 특성상 부활 직후 바로 이동하면 물리 엔진에 의해 스폰으로 팅길 수 있음
            -- 충분히 대기 (0.5초) 후 이동
            task.wait(0.5)
...
-- ==================== PHANTASM HUB FEATURES ====================

-- Upside Down
addcmd('upsidedown', {'upd'}, function(args, speaker)
    local char = speaker.Character
    if not char then
        notify('Error', 'Character를 찾을 수 없습니다')
        return
    ...
    -- Store track reference for later cancellation
    getgenv().UpsideDownTrack = track
    ...
    if root then
        local connection
        connection = game:GetService("RunService").Heartbeat:Connect(function()
            if not getgenv().UpsideDownActive or not root or not root.Parent then
                connection:Disconnect()
            ...
            pcall(function()
                ...
                root.CFrame = currentCFrame * CFrame.Angles(0, 0, math.rad(0.1))
            end)
        end)
    end
end)

addcmd('unupsidedown', {'unupd', 'noupd'}, function(args, speaker)
    getgenv().UpsideDownActive = false
    if getgenv().UpsideDownTrack then
        pcall(function() getgenv().UpsideDownTrack:Stop() end)
    end
    notify('Upside Down', '꺼졌습니다 ❌', 2)
end)

-- Anti Ragdoll
addcmd('antiragdoll', {'antirg', 'norag'}, function(args, speaker)
    local char = speaker.Character
    if not char then
        notify('Error', 'Character를 찾을 수 없습니다')
        return
    ...
    -- Monitor for ragdoll effects
    local ragdollItems = {"Freeze", "AntiMove", "Slowed", "StopRunning", "ComboStun"}
    local connection
    connection = game:GetService("RunService").Heartbeat:Connect(function()
        if not getgenv().AntiRagdollActive or not char or not char.Parent then
            connection:Disconnect()
            return
        end
        
        -- Remove ragdoll accessories
        for _, item in pairs(ragdollItems) do
            local ragdoll = char:FindFirstChild(item)
...
addcmd('unantiragdoll', {'unantirg', 'noantirag', 'norag'}, function(args, speaker)
    getgenv().AntiRagdollActive = false
    notify('Anti Ragdoll', '꺼졌습니다 ❌', 2)
end)

-- No Dash Cooldown
addcmd('nodashcooldown', {'nodc', 'fastdash'}, function(args, speaker)
...
addcmd('unnodashcooldown', {'unnodc', 'nonodc'}, function(args, speaker)
    getgenv().NoDashCooldownActive = false
    notify('No Dash Cooldown', '꺼졌습니다 ❌', 2)
    
    -- Reset workspace attribute
    pcall(function()
...
-- Flying
addcmd('fly', {'flight'}, function(args, speaker)
    ...
    local flySpeed = tonumber(args[1]) or 100
    local camera = workspace.CurrentCamera
    local flyRootCframe = root.CFrame
    
    local connection
    connection = game:GetService("RunService").Heartbeat:Connect(function(deltaTime)
        if not getgenv().FlyActive or not char or not char.Parent then
            connection:Disconnect()
        ...
        root.RotVelocity = Vector3.new(0, 0, 0)
        root.CFrame = CFrame.new(root.Position, root.Position + cameraLook)
    end)
end)

addcmd('unfly', {'unflight', 'nofly'}, function(args, speaker)
    getgenv().FlyActive = false
...
-- WalkFling (Velocity Spoof)
addcmd('walkfling', {'vspoof', 'spoof'}, function(args, speaker)
    local char = speaker.Character
    if not char then
        notify('Error', 'Character를 찾을 수 없습니다')
        return
    ...
    notify('WalkFling', '켜졌습니다 ✅', 2)
    
    local root = getRoot(char)
    if root then
        local connection
        connection = game:GetService("RunService").Heartbeat:Connect(function()
            if not getgenv().WalkFlingActive or not root or not root.Parent then
                connection:Disconnect()
                return
            end
            
            -- Spoof velocity to server for flinging
            pcall(function()
                root.Velocity = Vector3.new(math.random(-100000, 100000), math.random(-100000, 100000), math.random(-100000, 100000))
            end)
        end)
    end
end)

addcmd('unwalkfling', {'unvspoof', 'novspoof'}, function(args, speaker)
    getgenv().WalkFlingActive = false
    notify('WalkFling', '꺼졌습니다 ❌', 2)
end)

-- ==================== DEATH FLING (TOUCH FLING) ====================

local deathFlingEnabled = false
local deathFlingOldCFrame = nil
local deathFlingGUI = nil
local deathFlingButton = nil
...
task.spawn(function()
    ...
    local function toggleFling()
        deathFlingEnabled = not deathFlingEnabled
        if deathFlingButton then
            deathFlingButton.Text = deathFlingEnabled and "Fling: ON" or "Fling: OFF"
        end
        notify('Death Fling', 'Touch Fling ' .. (deathFlingEnabled and "활성화 ✅" or "비활성화 ❌"), 2)
    end
    getgenv().FLINGTOGGLE = toggleFling
    ...
    RunService.Heartbeat:Connect(function()
        ...
        -- NaN/High position check (Anti-Fling)
        local pos = root.Position
        if pos.X ~= pos.X or pos.Y ~= pos.Y or pos.Z ~= pos.Z or math.abs(pos.X) > 9e9 or math.abs(pos.Y) > 9e9 or math.abs(pos.Z) > 9e9 then
            if deathFlingOldCFrame then
                root.CFrame = deathFlingOldCFrame
            end
            return
        end
        deathFlingOldCFrame = root.CFrame
        
        -- Core Fling Logic
        if deathFlingEnabled then
...
local function ensureDeathFlingGUI()
    if deathFlingGUI then return end
    ...
    flingButton.Size = UDim2.new(0, 120, 0, 50)
    flingButton.Position = UDim2.new(0.5, -60, 0.5, -25)
    flingButton.Text = deathFlingEnabled and "Fling: ON" or "Fling: OFF"
    ...
    flingButton.MouseButton1Click:Connect(function()
        if getgenv().FLINGTOGGLE then
            getgenv().FLINGTOGGLE()
        end
    end)
    
    deathFlingGUI = screenGui
    deathFlingButton = flingButton
end

addcmd('walkdeathfling', {'wdf'}, function(args, speaker)
    deathFlingEnabled = true
    if deathFlingButton then
        deathFlingButton.Text = "Fling: ON"
    end
    notify('Death Fling', 'Walk Death Fling 활성화 ✅', 2)
end)

addcmd('unwalkdeathfling', {'unwdf'}, function(args, speaker)
    deathFlingEnabled = false
    if deathFlingButton then
        deathFlingButton.Text = "Fling: OFF"
    end
    notify('Death Fling', 'Walk Death Fling 비활성화 ❌', 2)
end)

addcmd('deathflinggui', {}, function(args, speaker)
    ensureDeathFlingGUI()
    deathFlingGUI.Enabled = not deathFlingGUI.Enabled
    notify('Death Fling', 'GUI가 켜졌습니다!!', 2)
end)

-- Emote Dash (Improved with Phantasm Hub logic)
addcmd('emotedash', {'edash'}, function(args, speaker)
    local char = speaker.Character
    if not char then
        notify('Error', 'Character를 찾을 수 없습니다')
        return
    ...
    local connection
    connection = game:GetService("RunService").Heartbeat:Connect(function()
        if not getgenv().EmoteDashActive or not char or not char.Parent then
            connection:Disconnect()
            return
        end
        
        -- Phantasm Hub Logic: Speed up dash animations
        for _, animTrack in pairs(humanoid:GetPlayingAnimationTracks()) do
            if table.find(dashAnimationIds, animTrack.Animation.AnimationId) then
                animTrack:AdjustSpeed(99)
            end
        end
        
        -- Fallback: If not specifically dashing but moving fast, play a subtle emote effect
        local root = getRoot(char)
        if root and root.Velocity.Magnitude > 50 and not getgenv().EmoteDashPlaying then
...
addcmd('unemotedash', {'unedash', 'noedash', 'noemotedash'}, function(args, speaker)
    getgenv().EmoteDashActive = false
    getgenv().EmoteDashPlaying = false
    notify('Emote Dash', '꺼졌습니다 ❌', 2)
end)

addcmd('resettp', {}, function(args, speaker)
    local char = speaker.Character
    if not char then return end
    local resetPos = getRoot(char).Position
    local humanoid = char:FindFirstChildWhichIsA("Humanoid")
    
    if humanoid then
        -- HP를 0으로 설정해서 리셋
        humanoid.Health = 0
        notify('Reset', 'Character를 리셋합니다...', 2)
        
        -- 리스폰 대기
        local spawnTime = 0
        while speaker.Character == char and spawnTime < 5 do
            wait(0.1)
        ...
        if newChar then
            ...
            if newRoot then
                newRoot.CFrame = CFrame.new(resetPos + Vector3.new(0, 3, 0))
                notify('Respawn', '리셋 위치로 이동했습니다', 2)
            end
        end
    end
end)
...
-- 리더보드 조작 시스템
local BanTargetName = nil
local BanActualName = nil
local BanDisplayName = nil
local FakeKills = 0
local FakeTotalKills = 0
local LeaderboardHijackEnabled = false

local function hijackLeaderboardUI(player)
    if not BanTargetName or not LeaderboardHijackEnabled then return end
    
    if player.Name == BanTargetName or player.DisplayName == BanTargetName or player.Name == BanActualName then
        -- 플레이어 객체 속성 직접 변조
        pcall(function()
            player.Name = BanActualName or BanTargetName
        end)
        
        pcall(function()
            player.DisplayName = BanDisplayName or BanActualName or BanTargetName
        end)
        ...
        if player.Character then
            ...
            if humanoid then
                pcall(function()
                    humanoid.DisplayName = BanDisplayName or BanActualName or BanTargetName
                end)
            end
        end
        ...
        if stats then
            for _, stat in pairs(stats:GetChildren()) do
                ...
                if FakeKills > 0 and (sName == "kill" or sName == "kills") then
                    pcall(function() stat.Value = FakeKills end)
                elseif FakeTotalKills > 0 and sName:find("total") and sName:find("kill") then
                    pcall(function() stat.Value = FakeTotalKills end)
                end
            end
        end
    end
...
local function forceUpdateUI()
    if not LeaderboardHijackEnabled or not BanTargetName then return end
    
    pcall(function()
        -- CoreGui 모든 리더보드 텍스트 변조
        local coreGui = game:GetService("CoreGui")
        for _, ui in pairs(coreGui:GetDescendants()) do
            if ui:IsA("TextLabel") or ui:IsA("TextButton") then
                -- 정확한 매칭
                if ui.Text == BanTargetName or ui.Text == BanActualName then
                    ui.Text = BanDisplayName or BanActualName or BanTargetName
                end
                -- 부분 매칭 (이름을 포함하는 경우)
                if string.find(ui.Text or "", BanTargetName or "") or string.find(ui.Text or "", BanActualName or "") then
                    if ui.Text ~= (BanDisplayName or BanActualName or BanTargetName) then
                        ui.Text = BanDisplayName or BanActualName or BanTargetName
                    end
                end
            end
    ...
    -- 플레이어 리스트 직접 변조
    pcall(function()
        local playerList = coreGui:FindFirstChild("RobloxPlayerList") or coreGui:FindFirstChild("PlayerList")
        if playerList then
            for _, v in pairs(playerList:GetDescendants()) do
                if v:IsA("TextLabel") or v:IsA("TextButton") then
                    if v.Text == BanTargetName or v.Text == BanActualName then
                        v.Text = BanDisplayName or BanActualName or BanTargetName
                    end
                end
            end
        end
    ...
    -- workspace의 모든 Humanoid 이름표도 변조
    pcall(function()
                    ...
                    if head then
                        for _, child in pairs(head:GetDescendants()) do
                            if child:IsA("TextLabel") then
                                child.Text = BanDisplayName or BanActualName or BanTargetName
                            end
                        end
                    end
                end
...
task.spawn(function()
    while task.wait(0.05) do
        if LeaderboardHijackEnabled then
            for _, p in pairs(game.Players:GetPlayers()) do
                hijackLeaderboardUI(p)
            end
            forceUpdateUI()
        end
    end
...
game.Players.PlayerAdded:Connect(function(player)
    task.wait(0.05)
    if LeaderboardHijackEnabled then
        hijackLeaderboardUI(player)
    end
end)

-- 플레이어 캐릭터 로드될 때 즉시 변조
game.Players.PlayerAdded:Connect(function(player)
    player.CharacterAdded:Connect(function(character)
        if LeaderboardHijackEnabled and (player.Name == BanTargetName or player.DisplayName == BanTargetName or player.Name == BanActualName) then
            task.wait(0.1)
            local humanoid = character:FindFirstChildWhichIsA("Humanoid")
            if humanoid then
                pcall(function()
...
-- 추가: Humanoid.DisplayName이 변경될 때마다 즉시 변조
task.spawn(function()
            ...
            for _, player in pairs(game.Players:GetPlayers()) do
                if player.Name == BanTargetName or player.DisplayName == BanTargetName or player.Name == BanActualName then
                    if player.Character then
                        ...
                        if humanoid then
                            -- 변경되었으면 즉시 다시 설정
                            if humanoid.DisplayName ~= (BanDisplayName or BanActualName or BanTargetName) then
                                pcall(function()
                                    humanoid.DisplayName = BanDisplayName or BanActualName or BanTargetName
                                end)
                            end
...
addcmd('banstart', {}, function(args, speaker)
    if not args[1] then
        notify('Error', '양식: /banstart alt username displayname kill totalkill')
        return
    end
    
    local altName = args[1]
    local username = args[2] or altName
    local displayname = username
    local killCount = 0
    local totalKillCount = 0
    
    -- args[3]이 숫자면 kill값, 문자면 displayname
    if args[3] then
        if tonumber(args[3]) then
            killCount = tonumber(args[3]) or 0
            totalKillCount = tonumber(args[4]) or 0
        else
            displayname = args[3]
            killCount = tonumber(args[4]) or 0
            totalKillCount = tonumber(args[5]) or 0
        end
    end
    
    BanTargetName = altName
    BanActualName = username
    BanDisplayName = displayname
    FakeKills = killCount
    FakeTotalKills = totalKillCount
    LeaderboardHijackEnabled = true
    ...
    notify('BanStart', 
        ...
        'Kill: ' .. killCount .. '\n' ..
        'Total Kill: ' .. totalKillCount, 4)
end)

addcmd('fakeclip', {}, function(args, speaker)
    if not args[1] then
        notify('Error', '양식: /fakeclip alt username displayname kill totalkill')
        return
    end
...
addcmd('antideathcountergui', {'adcgui'}, function(args, speaker)
    pcall(function()
...
-- [ 1. 안전 구역 파트 생성 ]
local function ensureVoidPart()
    local partName = "Unknown_Safety_Platform"
    local existingPart = workspace:FindFirstChild(partName)
    if not existingPart then
        local voidPart = Instance.new("Part")
        voidPart.Name = partName
        voidPart.Size = Vector3.new(100, 1.5, 100)
        voidPart.Anchored = true
        voidPart.CanCollide = true
        voidPart.Material = Enum.Material.Neon
        voidPart.Color = Color3.fromRGB(0, 255, 150)
        voidPart.CFrame = CFrame.new(-363.4, -501.2, 643.7)
...
execButton.Size = UDim2.new(0, 210, 0, 50)
execButton.Position = UDim2.new(0, 20, 0, 60)
execButton.Text = "실행"
...
Instance.new("UICorner", execButton).CornerRadius = UDim.new(0, 8)
...
execButton.MouseButton1Click:Connect(function()
    ...
    if hrp and hum then
        local originalCFrame = hrp.CFrame
        local safetyPart = ensureVoidPart()
        
        -- 안전 구역 이동
        hrp.CFrame = safetyPart.CFrame + Vector3.new(0, 5, 0)
        
        task.wait(2.5) -- 컷신이 끝날 때까지 충분한 시간 대기
        
        -- 원래 위치로 복귀
        hrp.CFrame = originalCFrame
        
        -- ⭐ 카메라 잠금 강제 해제 로직 ⭐
        cam.CameraType = Enum.CameraType.Custom -- 우선권을 플레이어에게 반환
        cam.CameraSubject = hum -- 대상을 다시 캐릭터로 강제 지정
        
        -- 일부 컷신 스크립트가 CameraType을 계속 고정할 경우를 대비해 한 번 더 호출
...
addcmd('crabadd', {'crabspawn'}, function(args, speaker)
    local char = speaker.Character
    if not char then
        notify('Error', 'Character를 찾을 수 없습니다')
        return
...
local OFFSET = CFrame.new(1e-1000, 1e-1000, 1e-1000) -- 유저님의 핵심 오프셋
...
RunService.Heartbeat:Connect(function(dt)
    if not isOrbiting or not orbitTarget or not orbitTarget.Parent then return end
    ...
    if root then
        ...
        local targetPos = orbitTarget.Position + Vector3.new(offsetX, 0, offsetZ)
        
        -- 2. 시선 처리와 1e-1000 오프셋 결합
        -- CFrame.lookAt으로 방향을 잡고, 그 결과에 유저님의 OFFSET을 곱해 물리 딜레이를 제거합니다.
        local finalCFrame = CFrame.lookAt(targetPos, orbitTarget.Position) * OFFSET
...
addcmd('orbit', {}, function(args, speaker)
    if args[1] then
        ...
        if targetPlayers and #targetPlayers > 0 then
            local p = Players:FindFirstChild(targetPlayers[1])
            if p and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                orbitTarget = p.Character.HumanoidRootPart
                isOrbiting = true
                notify('Orbit', p.Name .. ' 대상 추적 공전 시작', 3)
            end
        end
    else
...
-- 설정값
local OFFSET = CFrame.new(1e-1000, 1e-1000, 1e-1000)
local isRunning = false
local currentTarget = nil
local originalCFrame = nil -- 복귀용 좌표 저장
local oldPosCFrame = nil    -- NaN 방지용 좌표 저장

-- [핵심 1] NaN 방지 및 데스플링 물리 처리 (Heartbeat)
RunService.Heartbeat:Connect(function()
    if not isRunning or not lp.Character then return end
    
    local root = lp.Character:FindFirstChild("HumanoidRootPart")
    local hum = lp.Character:FindFirstChildOfClass("Humanoid")
    if not root or not hum then return end

    -- 1. 마스터 위치 이탈(NaN/Infinity) 방지
    local pos = root.Position
    if pos.X ~= pos.X or math.abs(pos.X) > 9e9 then
        root.CFrame = oldPosCFrame or root.CFrame
        return
    end
    oldPosCFrame = root.CFrame

    -- 2. 데스플링 물리 주입 및 타겟 유착
    if currentTarget and currentTarget.Parent then
        pcall(function()
            -- 고죠 인피니티(NaN) 주입
            sethiddenproperty(hum, "MoveDirectionInternal", Vector3.new(0/0, 0/0, 0/0))
            -- 타겟에게 1e-1000 오프셋으로 초밀착
            root.CFrame = currentTarget.CFrame * OFFSET
        end)
    else
        pcall(function()
            -- 타겟이 없을 때는 물리 초기화
...
-- [핵심 2] addcmd 실행부
addcmd('killall', {}, function(args, speaker)
    ...
    -- 시작 전 상태 저장
    originalCFrame = root.CFrame -- 복귀할 원래 위치 저장
    isRunning = true
    
    task.spawn(function()
        local eliminated = {}
        local startTime = tick()
        notify('KillAll', '킬올을 시작합니다', 3)

        for _, plr in ipairs(Players:GetPlayers()) do
            -- 20초 제한 또는 중단 체크
            if not isRunning or (tick() - startTime > 20) then break end
            
            if plr ~= lp and not eliminated[plr.UserId] then
                local char = plr.Character
                ...
                if char and hum and hum.Health > 0 and hrp then
                    ...
                    -- 0.5초간 타겟팅
                    local killStep = tick()
                    while isRunning and (tick() - killStep < 0.5) do
                        if not (char and hum and hum.Health > 0 and hrp.Parent) then break end
                        task.wait()
                    end
        ...
        pcall(function()
            local hum = lp.Character:FindFirstChildOfClass("Humanoid")
            if hum then sethiddenproperty(hum, "MoveDirectionInternal", Vector3.new(0, 0, 0)) end
        end)

        -- 원래 자리로 복귀
        if originalCFrame and lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
...
-- 강제 중단 시에도 복귀하게 설정
addcmd('unkillall', {}, function(args, speaker)
    isRunning = false
    currentTarget = nil
    if originalCFrame and lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
        lp.Character.HumanoidRootPart.CFrame = originalCFrame
...
-- 유저님의 성공 공식 수치
local OFFSET = CFrame.new(1e-1000, 1e-1000, 60e-1)

local isAttached = false
local attachTarget = nil
local targetPlayerObj = nil -- 타겟 플레이어 객체 저장용
local charAddedConn = nil   -- 캐릭터 추가 이벤트 연결용
...
-- [핵심 루프] 물리 유착 처리
RunService.Heartbeat:Connect(function()
    if not isAttached or not attachTarget or not attachTarget.Parent or not RootPart then 
        return 
    end
    
    pcall(function()
        -- 물리 엔진 유착 주입
        sethiddenproperty(RootPart, "PhysicsRepRootPart", attachTarget)
    end)
    
    -- 6스터드 뒤쪽 + 1e-1000 딜레이 제거 적용
    RootPart.CFrame = attachTarget.CFrame * OFFSET
end)

-- [기능] 타겟 플레이어 설정 및 자동 재부착 로직
local function setTarget(plr)
    if charAddedConn then charAddedConn:Disconnect() end -- 기존 연결 해제
    ...
    if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
        attachTarget = plr.Character.HumanoidRootPart
    end
    
    -- [자동 재부착 핵심] 타겟이 죽고 다시 태어날 때 감지
    charAddedConn = plr.CharacterAdded:Connect(function(newChar)
        local newHrp = newChar:WaitForChild("HumanoidRootPart", 5)
        if newHrp then
            attachTarget = newHrp
            notify('Attach', plr.Name .. ' 부활 감지, 재부착 완료', 2)
        end
    end)
end

-- [명령어 등록]
addcmd('attach', {'looptp', 'at'}, function(args, speaker)
    if args[1] then
        local targetPlayers = getPlayer(args[1], speaker)
        if targetPlayers and #targetPlayers > 0 then
            local p = Players:FindFirstChild(targetPlayers[1])
            if p then
                setTarget(p)
                notify('Attach', p.Name .. ' 유착 및 자동 추적 시작', 3)
            end
        end
    else
        notify('오류', '대상을 입력해주세요.', 3)
...
-- [해제 명령어]
addcmd('unattach', {'unlooptp', 'unat'}, function(args, speaker)
    isAttached = false
    attachTarget = nil
    targetPlayerObj = nil
...
addcmd('killall', {}, function(args, speaker)
    ...
    notify('KillAll', '완료 및 복귀', 3)
end)

--[[
    IY Custom Command: kill
    기능: PhysicsRepRootPart를 활용한 물리적 킬 및 복귀
]]

addcmd('kill', {'die', 'pk'}, function(args, speaker)
    local targetPlayers = getPlayer(args[1], speaker)
    ...
    for _, v in pairs(targetPlayers) do
        if v == LocalPlayer then continue end -- 자신은 제외
        ...
        if char and hrp and hum and hum.Health > 0 then
            ...
            if lpHrp and lpHum then
                -- 1. 위치 저장 (명령어 실행 시점)
                originalCFrame = lpHrp.CFrame
                
                -- 2. 대상에게 접근 및 Physics Attach 실행
                notify("Kill 진행 중", v.DisplayName .. " 제거 중...", 2)
                
                local connection
                local startTime = tick()
                
                -- 하트비트 루프를 통한 강력한 플링(Kill) 적용
                connection = game:GetService("RunService").Heartbeat:Connect(function()
                    if not hrp or not hrp.Parent or hum.Health <= 0 or (tick() - startTime > 3) then
                        connection:Disconnect()
                        return
                    end
                    ...
                    -- Physics Attack 메커니즘
                    pcall(function()
                        sethiddenproperty(lpHrp, "PhysicsRepRootPart", hrp)
                        sethiddenproperty(lpHum, "MoveDirectionInternal", Vector3.new(0/0, 0/0, 0/0))
                    end)
                    lpHrp.CFrame = hrp.CFrame * CFrame.new(1e-100, 1e-100, 1e-100)
                end)

                -- 대상이 죽을 때까지 대기 (최대 3초)
                repeat task.wait() until hum.Health <= 0 or (tick() - startTime > 3)
                
                -- 3. 뒷정리 (Physics 해제)
                pcall(function()
                    sethiddenproperty(lpHrp, "PhysicsRepRootPart", nil)
                end)
                ...
                notify("완료", v.DisplayName .. " 처치 완료 및 복귀", 2)
            end
        else
            notify("스킵", v.DisplayName .. "님은 이미 사망 상태이거나 무효합니다.", 2)
        end
    end
end)
...
Players.LocalPlayer.Chatted:Connect(function(msg)
    if msg:sub(1, #prefix) == prefix then
        local cmdText = msg:sub(#prefix + 1)
        execCmd(cmdText, Players.LocalPlayer, true)
    end
end)
...
-- Passive Anti-Void System (Silent & Persistent)
task.spawn(function()
    local Players = game:GetService("Players")
    local RunService = game:GetService("RunService")
    local lp = Players.LocalPlayer
    ...
    local voidThreshold = -500 -- The Y level at which anti-void triggers
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if root and humanoid and humanoid.Health > 0 then
            ...
            -- If we are above the void threshold, update the last safe position
            if pos.Y > voidThreshold then
                -- Only update if we are not moving too fast (to avoid saving a falling position)
                if root.Velocity.Magnitude < 100 then
                    lastSafePosition = root.CFrame
                end
            else
                -- We fell! Teleport back to the last safe position
                if lastSafePosition then
                    root.Velocity = Vector3.new(0, 0, 0)
                    root.CFrame = lastSafePosition + Vector3.new(0, 3, 0)
                else
                    -- Fallback if no safe position was saved
                    root.Velocity = Vector3.new(0, 0, 0)
